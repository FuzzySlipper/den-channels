...
- `includeOrdinaryMemberships=true` is accepted as a legacy no-op; do not rely on it for filtering.
...
Long-lived Runner/Planner-style agents follow the same read model. Their ordinary channel memberships are stable residency, not task-target worker residency, so they should not be mislabeled as `target_work` just to make discovery work.
...
curl 'http://<channels-host>/api/channel-memberships?memberIdentity=den-mcp-runner'
```
...
- long-lived runtime agents show their ordinary null-purpose channel memberships by default;
